# NextAuth Default Signin Page Source Code

**Source**: `secretus/next-auth/packages/core/src/lib/pages/signin.tsx`
          ...
          return (
            <div key={provider.id} className="provider">
              ...
              {provider.type === "email" && (
                <form action={provider.signinUrl} method="POST">
                  <input type="hidden" name="csrfToken" value={csrfToken} />
                  <label className="section-header" htmlFor={`input-email-for-${provider.id}-provider`}>
                    Email
                  </label>
                  <input
              ...
              {provider.type === "webauthn" && (
                <form action={provider.callbackUrl} method="POST" id={`${provider.id}-form`}>
                  ...
                  {Object.keys(provider.formFields).map((field) => {
                    return (
                      <div key={`input-group-${provider.id}`}>
                        <label className="section-header" htmlFor={`input-${field}-for-${provider.id}-provider`}>
...
### Provider Types Rendered:

1. **OAuth/OIDC**: Form POST to `provider.signinUrl` with CSRF token
2. **Email**: Email input form for magic link authentication
3. **Credentials**: Dynamic form fields based on provider configuration
4. **WebAuthn**: Passkey/biometric authentication with conditional UI
...
- Provider logo loading from AuthJS CDN
